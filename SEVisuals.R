...
FlightPerOther <- ggplot(project_other,aes(x=project_other$No.of.Flights.p.a.,y=project_other$Satisfaction))+geom_count()+
  stat_summary(aes(y =project_other$Satisfaction,group=1), fun.y=mean, colour="red", geom="point",group=1)

#13.Satisfaction vs.class for SE
ClassSE <- ggplot(project_SE,aes(x=project_SE$Class,y=project_SE$Satisfaction))+geom_count()+
  stat_summary(aes(y =project_SE$Satisfaction,group=1), fun.y=mean, colour="red", geom="point",group=1)

#14.Satisfaction vs. class for all other airlines
ClassOther <- ggplot(project_other,aes(x=project_other$Class,y=project_other$Satisfaction))+geom_count()+
  stat_summary(aes(y =project_other$Satisfaction,group=1), fun.y=mean, colour="red", geom="point",group=1)

#15.Satisfaction vs. Price Sensitivity for SE
PriceSenSE <- ggplot(project_SE,aes(x=project_SE$Price.Sensitivity,y=project_SE$Satisfaction))+geom_count()+
  stat_summary(aes(y =project_SE$Satisfaction,group=1), fun.y=mean, colour="red", geom="point",group=1)
